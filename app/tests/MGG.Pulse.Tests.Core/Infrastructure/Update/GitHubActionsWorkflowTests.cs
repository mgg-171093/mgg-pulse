...
{
    [Fact]
    public void CiWorkflow_UsesHostedRunnerSafePipeline_WithCoreProjectBoundary()
    {
        ...
        Assert.Contains("dotnet test app/tests/MGG.Pulse.Tests.Core/MGG.Pulse.Tests.Core.csproj --configuration Release --no-build", normalized, StringComparison.Ordinal);
        Assert.DoesNotContain("--filter", normalized, StringComparison.Ordinal);
    ...
    [Fact]
    public void ThemeServiceTests_AreIsolatedInUiOnlyTestProject()
    {
        var tests = File.ReadAllText(ResolveRepoFilePath("app", "tests", "MGG.Pulse.Tests.UI", "UI", "Services", "ThemeServiceTests.cs"));

        Assert.Contains("namespace MGG.Pulse.Tests.UI.Services;", tests, StringComparison.Ordinal);
    }

    [Fact]
    public void TestProjects_AndReadme_DocumentProjectSplitContract()
    {
        var coreTestProject = File.ReadAllText(ResolveRepoFilePath("app", "tests", "MGG.Pulse.Tests.Core", "MGG.Pulse.Tests.Core.csproj"));
        var uiTestProject = File.ReadAllText(ResolveRepoFilePath("app", "tests", "MGG.Pulse.Tests.UI", "MGG.Pulse.Tests.UI.csproj"));
        var readme = File.ReadAllText(ResolveRepoFilePath("README.md"));

        Assert.DoesNotContain("MGG.Pulse.UI", coreTestProject, StringComparison.Ordinal);
        Assert.Contains("MGG.Pulse.UI", uiTestProject, StringComparison.Ordinal);
        Assert.Contains("MGG.Pulse.Tests.Core", readme, StringComparison.Ordinal);
        Assert.Contains("MGG.Pulse.Tests.UI", readme, StringComparison.Ordinal);
